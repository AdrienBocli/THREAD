...
int main()
{
	...
	sigaction(SIGUSR1,&A,NULL);
 	...
 	ret=pthread_cancel(threadHandle5);

 	ret= pthread_join(threadHandle5, (void **)&retThread5);
...
void * fctThreadMaster ()
{
	...
	pthread_cleanup_push(fctTerminaison,NULL);
...
void fctTerminaison(void *p)
{
	printf("\nLe thread master se termine no %u\n",pthread_self());
}